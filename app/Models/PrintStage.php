...
class PrintStage extends Model
{
    use HasFactory, HasOptimisticLocking, Auditable;

    protected $table = 'print_stages';

    public const STAGES = ['TRANS', 'PREP', 'DESIGN', 'NESTING', 'PRINT', 'POST', 'QC'];
    ...
    {
        return in_array($this->stage, ['NESTING', 'PRINT']);
    }

    public function canComplete(): bool
    {
        if ($this->requiresPrinter() && empty($this->printer)) {
            return false;
        }
        return true;
    }
