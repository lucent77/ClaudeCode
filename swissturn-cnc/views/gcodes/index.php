<div class="flex justify-between items-center mb-8">
    <h1 class="text-3xl font-bold">G-Codes</h1>
    <a href="/gcodes/create"
...
<?php if (empty($gcodes)): ?>
    <div class="text-center py-12 bg-cnc-blue rounded-lg">
        <p class="text-gray-400 mb-4">No G-codes found</p>
        <a href="/gcodes/create" class="text-cnc-highlight hover:underline">Add your first G-code</a>
    </div>
<?php else: ?>
        ...
        <table class="w-full">
            <thead class="bg-cnc-accent">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left">Modal Group</th>
                    <th class="px-4 py-3 text-left">Type</th>
                    <th class="px-4 py-3 text-left w-24">Actions</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-cnc-accent">
                <?php foreach ($gcodes as $gcode): ?>
                    <?php $machine = $gcode->machine_id ? \SwissTurn\Models\Machine::find($gcode->machine_id) : null; ?>
                    <tr class="hover:bg-cnc-accent/50 transition">
                        <td class="px-4 py-3">
                            <span class="font-mono text-cnc-highlight font-bold"><?= htmlspecialchars($gcode->code) ?></span>
                        </td>
                        <td class="px-4 py-3">
                            <div><?= htmlspecialchars($gcode->name) ?></div>
                            <?php if ($gcode->description): ?>
                                <div class="text-gray-500 text-sm truncate max-w-md">
                                    <?= htmlspecialchars($gcode->description) ?>
                                </div>
                            <?php endif; ?>
                        </td>
                        ...
                        <td class="px-4 py-3 text-gray-400">
                            <?= $gcode->modal_group_id ?? '-' ?>
                            <?= $gcode->is_modal ? '' : '<span class="text-xs text-yellow-400">(non-modal)</span>' ?>
                        </td>
                        <td class="px-4 py-3 text-gray-400">
                            <?= htmlspecialchars($gcode->semantic_type ?? '-') ?>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex gap-2">
                                <a href="/gcodes/<?= $gcode->id ?>/edit"
                                ...
                                <form action="/gcodes/<?= $gcode->id ?>/delete" method="post"
                                      onsubmit="return confirm('Delete this G-code?')" class="inline">
